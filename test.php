...
$rand_nums = array();
for ($i = 0; $i < 20000; ++$i) {
    $rand_nums[$i] = random_int(0, 100000);
}
...
foreach ($sorters as $sorter) {
    $nums = array_slice($tosort_nums, 0);
    $sorter->sort($nums);
    $identical = $nums === $sorted_nums ? "true" : "false";
    echo "{$sorter->getName()} Sort of predefined array is correct: $identical\n";

    $nums = array_slice($rand_nums, 0);
    $watch->start();
    $sorter->sort($nums);
    $milliseconds = $watch->getElapsedMilliseconds();
    $sorted = isSorted($nums) ? "true" : "false";
    echo "{$sorter->getName()} Sort of random array finished in $milliseconds milliseconds. Sorted: $sorted\n";
}
?>